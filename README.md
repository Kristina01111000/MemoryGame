# MemoryGame
Basic memory game using javascript

- figure out how to update from atom

8 images needed, 100 by 100 px, they are stored in the first issue -> change the pictures to something better later

to do:
- make the cards show up on screen
- make them flip when clicked
- make them disappear when matched 
- make them appear in randomized order
- have a game reset button
- add to score when matched
